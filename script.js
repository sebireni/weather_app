async function loadEvent() {
    ...
    let searchUrl = "http://api.weatherapi.com/v1/search.json?key=7874ced20d8548e4ab5195616212206&q=";
    let currentUrl = "http://api.weatherapi.com/v1/current.json?key=7874ced20d8548e4ab5195616212206&q=";
    let iconUrl = "";
    ...
    function updateCityValue(event) {
        ...
        fetch(currentUrl + input)
           ...
           .then(
               function(data)  {
                   function output(){
                       fetch("https://api.pexels.com/v1/search?query=" + data.location.name + "%20city", {
                           headers: {
                               Authorization: "563492ad6f91700001000001b756c258bfb449658e3f81544e8b08df"
                           }
                        })
                        .then(
                            function(response) {
                            ...
                            .then(
                                function(photoData) {
                                    ...
                                    document.querySelector("#cityBG").setAttribute("src", photoData.photos[0].src.portrait);
                                }
                                )
                        spinner.setAttribute('hidden', '');
                        document.querySelector("#temperatureE").innerHTML = data.current.temp_c + " °C";
                        document.querySelector("#skyConditionE").innerHTML = data.current.condition.text;
                        ...
                        document.querySelector("#humidityE").innerHTML = "humidity:  " + data.current.humidity + "%";
