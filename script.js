async function loadEvent() {
  ...
  const searchUrl = `http://api.weatherapi.com/v1/search.json?key=${KEY}&q=`;
  const currentUrl = `http://api.weatherapi.com/v1/current.json?key=${KEY}&q=`;
  ...
  const showImage = (locationName) => {
    fetch(`https://api.pexels.com/v1/search?query=${locationName}%20city`, {
      ...
      .then((response) => response.json())
      .then((photoData) => document.querySelector('#cityBG').setAttribute('src', photoData.photos[0].src.portrait))
  };
